...
export const createBooks = async (req: Request, res: Response):Promise<any> => {
    try {
...
export const updateBooks=async(req:Request,res:Response):Promise<any>=>{
    try {
        ...
        const [updatedBook]:any[] = await sequelize.query(
            "UPDATE books SET total_copies=? where isbn=?",{replacements:[copies,isbn]}
        );
